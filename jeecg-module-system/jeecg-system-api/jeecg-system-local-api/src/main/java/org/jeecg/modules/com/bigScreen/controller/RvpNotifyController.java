...
//            keyName = "Line:" + queryChannel + ":" + name;
    ...
    //wy106折线图
    @GetMapping(value = "/successByWy106")
    public JSONObject successByWy106(HttpServletRequest request, HttpServletResponse response) {
        JSONObject jsonObject = new JSONObject();
        try {
            JSONObject resultObject = new JSONObject();
            String[] categories = new String[5];
            String[] unKnownPercent = new String[5];
            String[] successPercent = new String[5];
            String[] failPercent = new String[5];
//            List<ChannelLineDomain> alls = null;
//            List<ChannelLineDomain> success = null;
            Long all = 0l;
            Long success = 0l;
...
//        String keyName = "Line:" + queryChannel + ":" + endTime;
            ...
            if (redisUtil.hasKey(keyName)) {
                ...
                List<String> jsonArrayToStringList = JSONObject.parseArray(o.toString(), String.class);
                String[] dateList = gson.fromJson(jsonArrayToStringList.get(0), String[].class);
                String[] unKnownList = gson.fromJson(jsonArrayToStringList.get(1), String[].class);
                String[] successList = gson.fromJson(jsonArrayToStringList.get(2), String[].class);
                String[] failList = gson.fromJson(jsonArrayToStringList.get(3), String[].class);
                for (int i = 0; i < dateList.length - 1; i++) {
                    categories[i] = dateList[i + 1].toString();
                }
                for (int i = 0; i < unKnownList.length - 1; i++) {
                    unKnownPercent[i] = unKnownList[i + 1].toString();
                }
                for (int i = 0; i < successList.length - 1; i++) {
                    successPercent[i] = successList[i + 1].toString();
                }
                for (int i = 0; i < failList.length - 1; i++) {
                    failPercent[i] = failList[i + 1].toString();
                }

                all = rvpNotifyService.queryCountAllByWy(endTime);
                success = rvpNotifyService.querySuccessByWy(endTime, true);
                returnNum = rvpNotifyService.querySuccessByWy(endTime, false);
                categories[4] = endTime.substring(11);

                unKnownPercent[4] = String.valueOf(getPercent(all - returnNum, all));
                successPercent[4] = String.valueOf(getPercent(success, all));
                failPercent[4] = String.valueOf(getPercent(returnNum - success, all));
                ...
                for (int i = 0; i < times.size(); i++) {
                    ...
                    success = rvpNotifyService.querySuccessByWy(endTime, true);
                    returnNum = rvpNotifyService.querySuccessByWy(endTime, false);
                    unKnownPercent[i] = String.valueOf(getPercent(all - returnNum, all));
                    successPercent[i] = String.valueOf(getPercent(success, all));
                    failPercent[i] = String.valueOf(getPercent(returnNum - success, all));
                }
            }


            resultObject.put("dateList", categories);
            resultObject.put("unKnownPercent", unKnownPercent);
            resultObject.put("successPercent", successPercent);
